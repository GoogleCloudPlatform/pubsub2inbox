...
from _vendor.python_docker import utils
...
def _parse_v1_layer(tar, layer_id):
    from _vendor.python_docker.base import Layer
...
def write_v1_layer_metadata(layer):
    ...
    return utils.sorted_json_dumps(metadata)


def write_v1_repositories(image):
    return utils.sorted_json_dumps(
        {image.name: {
            image.tag: image.layers[0].id
        }})
